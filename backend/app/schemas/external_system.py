...
class ExternalSystemBase(BaseModel):
    ...
    external_system_id: str = Field(
        ..., description="External system's real ID (e.g., shop1.myshopify.com)"
    )
    base_url: Optional[str] = Field(None, description="API base URL")
    webhook_url: Optional[str] = Field(None, description="Webhook endpoint URL")
    credentials: Dict[str, Any] = Field(
        default_factory=dict, description="System credentials"
    )
    settings: Dict[str, Any] = Field(
        default_factory=dict, description="System-specific settings"
    )
    ...
    sync_interval_minutes: Optional[int] = Field(
        60, description="Sync interval in minutes"
    )
...
class ExternalSystemUpdate(BaseModel):
    ...
    external_system_id: Optional[str] = Field(
        None, description="External system's real ID (e.g., shop1.myshopify.com)"
    )
    base_url: Optional[str] = Field(None, description="API base URL")
    webhook_url: Optional[str] = Field(None, description="Webhook endpoint URL")
    credentials: Optional[Dict[str, Any]] = Field(
        None, description="System credentials"
    )
    settings: Optional[Dict[str, Any]] = Field(
        None, description="System-specific settings"
    )
    sync_enabled: Optional[bool] = Field(None, description="Enable automatic sync")
    webhook_enabled: Optional[bool] = Field(
        None, description="Enable webhook processing"
    )
    sync_interval_minutes: Optional[int] = Field(
        None, description="Sync interval in minutes"
    )

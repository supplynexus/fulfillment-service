...
class ExternalSystemService:
    ...
    async def create_external_system(
        ...
        try:
            ...
            if "uq_tenant_system_external_id" in str(e):
                raise ValueError(
                    f"External system with ID '{external_system_id}' already exists for this tenant and system type"
                )
            else:
    ...
    async def get_external_system(
        self, external_system_id: int, tenant_id: Optional[int] = None
    ...
    async def get_external_system_by_external_id(
        self, external_id: str, tenant_id: int
    ) -> Optional[ExternalSystem]:
    ...
    async def get_external_systems_by_type(
        self, tenant_id: int, system_type: ExternalSystemType, active_only: bool = True
    ) -> List[ExternalSystem]:
    ...
    async def update_external_system(
        self, external_system_id: int, tenant_id: int, **kwargs
    ) -> Optional[ExternalSystem]:
    ...
    async def update_sync_status(
        ...
        result = await self.update_external_system(
            external_system_id, tenant_id, **update_data
        )
    ...
    async def get_active_sync_systems(
        ...
        query = select(ExternalSystem).where(
            ExternalSystem.is_active == True, ExternalSystem.sync_enabled == True
        )

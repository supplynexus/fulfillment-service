...
class OrderRoutingService:
    ...
    async def route_order_to_scm(
        ...
        try:
            ...
            for decision in routing_decisions:
                scm_order = await self._create_scm_order(
                    order, 
                    decision,
                    tenant_id
                )
    ...
    def _analyze_line_items(self, line_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """分析订单商品"""
        analyzed_items = []
    ...
    async def _apply_routing_rules(
        ...
        if routing_config.routing_strategy == "manual":
            ...
            # 自动路由：基于规则自动分配
            routing_decisions = await self._auto_route_by_rules(
                line_items, 
                tenant_id
            )
            ...
            # 混合路由：结合自动和手动
            auto_decisions = await self._auto_route_by_rules(
                line_items, 
                tenant_id
            )
            manual_decisions = []
    ...
    def _item_matches_rule(self, item: Dict[str, Any], conditions: Dict[str, Any]) -> bool:
        """检查商品是否匹配规则条件"""
        # 产品类型匹配

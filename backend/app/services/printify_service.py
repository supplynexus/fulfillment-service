...
class PrintifyService:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                return {
                    ...
                    "shops_count": len(shops_data) if isinstance(shops_data, list) else 0,
                    "shops": shops_data[:5] if isinstance(shops_data, list) else [],  # Return first 5 shops
                    "api_version": "v1",
                    "base_url": self.base_url
                }
        except httpx.HTTPStatusError as e:
            logger.error(f"Printify API HTTP error: {e.response.status_code} - {e.response.text}")
            return {
                "success": False,
    ...
    async def get_orders(self, shop_id: str, limit: int = 50, page: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Printify API HTTP error getting orders: {e.response.status_code} - {e.response.text}")
            return {
                "success": False,
                "message": f"Printify API error: {e.response.status_code}",
                "error_code": e.response.status_code,
    ...
    async def create_shipping_label_from_shopify_order(
        ...
        try:
            ...
            # Build customer name
            first_name = customer.get("first_name", shipping_address.get("first_name", ""))
            ...
            for item in line_items:
                ...
                if printify_product_mapping:
                    product_id = printify_product_mapping.get(str(item.get("product_id", "")))
                    if product_id:
            ...
            logger.info(f"Creating Printify order for Shopify order {shopify_order.get('id')}")
            ...
            if result:
                ...
                logger.error(f"Failed to create Printify order for Shopify order {shopify_order.get('id')}")

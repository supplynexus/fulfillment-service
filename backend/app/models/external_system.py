...
class ExternalSystem(Base):
    ...
    )  # Human readable name (e.g., "Main Shopify Store", "Printify Production")

    # External system configuration
    external_id = Column(
        String, nullable=True
    )  # External system's ID for this connection (deprecated)
    external_system_id = Column(
        String(100), nullable=False
    )  # External system's real ID (e.g., shop1.myshopify.com)

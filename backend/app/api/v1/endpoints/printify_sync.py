...
async def get_sync_status(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"🔍 获取到商品数量: {len(products)}")
            
            # 统计信息
            total_products = len(products)
            synced_products = len([p for p in products if getattr(p, 'sync_status', 'pending') == 'synced'])
            pending_products = len([p for p in products if getattr(p, 'sync_status', 'pending') == 'pending'])
            error_products = len([p for p in products if getattr(p, 'sync_status', 'pending') == 'error'])
            
            logger.info(f"🔍 同步状态统计: 总计={total_products}, 已同步={synced_products}, 待处理={pending_products}, 错误={error_products}")
        except Exception as e:
        ...
        return {
            ...
            "sync_rate": round(synced_products / total_products * 100, 2) if total_products > 0 else 0
        }

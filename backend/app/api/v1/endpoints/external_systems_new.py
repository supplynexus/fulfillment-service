...
@router.get("/shopify/{shop_id}/products")
async def get_shopify_products(
    ...
    # Get Shopify system configuration to get access token
    service = ExternalSystemService(db)
    ...
    if not external_system:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Shopify store not found"
        )
    
    if external_system.system_type != ExternalSystemType.SHOPIFY:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="External system is not a Shopify store"
        )
    
    # Get products using ShopifyService
    from app.services.shopify_service import ShopifyService
    shopify_service = ShopifyService(db)
    
    try:
        access_token = external_system.credentials.get("access_token")
        if not access_token:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        result = await shopify_service.get_products(
            shop_id=shop_id,
            access_token=access_token,
        ...
        if result["success"]:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=result.get("error", "Failed to get products")
            )
...
@router.get("/shopify/{shop_id}/orders")
async def get_shopify_orders(
    ...
    # Get orders using ShopifyService
    from app.services.shopify_service import ShopifyService
    shopify_service = ShopifyService(db)
    ...
    try:
        ...
        result = await shopify_service.get_orders(
            shop_id=shop_id,
            access_token=access_token,
            api_version=external_system.settings.get('api_version', '2024-10')
        )
        ...
        if result["success"]:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=result.get("error", "Failed to get orders")
            )

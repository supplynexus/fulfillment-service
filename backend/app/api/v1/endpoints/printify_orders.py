...
async def get_printify_config(db: AsyncSession, tenant_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        config = {
            ...
            "shop_id": printify_system.external_system_id or credentials.get("shop_id"),
...
async def create_printify_order(
    ...
    try:
        ...
        # 构建Printify订单数据
        external_id = f"ORDER_{request.order_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code in [200, 201]:
                ...
                return PrintifyOrderResponse(
                    ...
                    total_price=result.get("total_price", 0) / 100 if result.get("total_price") else 0,
                    message="Printify订单创建成功"
                )
            else:
                error_detail = response.text
                logger.error("❌ Printify API调用失败", 
                           status_code=response.status_code,
                           error_detail=error_detail)
                
                raise HTTPException(
                    status_code=400,
                    detail=f"Printify API调用失败: {response.status_code} - {error_detail}"
                )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.error("❌ 获取Printify商品列表失败", status_code=response.status_code)
                raise HTTPException(
                    status_code=400,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"获取Printify商品列表失败: {str(e)}"
        )

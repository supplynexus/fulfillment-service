...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL", "postgresql+asyncpg://localhost:5433/supplynexus"
    )
    ...
    WEBHOOK_SECRET: str = os.getenv(
        "WEBHOOK_SECRET", "dev-webhook-secret-change-in-prod"
    )
    ...
    # 健康检查配置
    HEALTH_CHECK_API_KEY: Optional[str] = os.getenv(
        "HEALTH_CHECK_API_KEY"
    )  # 必须配置，无缺省值
    HEALTH_CHECK_RATE_LIMIT: int = int(
        os.getenv("HEALTH_CHECK_RATE_LIMIT", "10")
    )  # 每分钟请求数
    HEALTH_CHECK_RATE_WINDOW: int = int(
        os.getenv("HEALTH_CHECK_RATE_WINDOW", "60")
    )  # 时间窗口（秒）
    ...
    def get_env_file_path(self) -> str:
        ...
        deployment_path = os.path.join(
            current_dir, "..", "deployment", "environments", "backend", env_file
        )

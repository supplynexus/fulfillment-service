...
AUTOMATION_STEPS_SEED = [
    ...
    {
        "step_key": "create_fulfillment_orders",
        "name": "生成履约订单（如Printify）",
        "description": "从SCM订单生成履约订单（如Printify订单）",
        "category": "order_processing",
    ...
    },
    {
        ...
        "name": "履约订单状态同步到SCM",
        "description": "从履约系统（如Printify）同步订单状态到SCM订单",
...
MANUAL_BUTTONS_SEED = [
    ...
    {
        "step_key": "create_fulfillment_orders",
        "button_key": "fulfill_to_printify",
        "button_label": "发送到Printify",
        "button_action": "/api/v1/scm-orders/{id}/fulfill",
        "http_method": "POST",
        "is_recommended": False,
        "is_deprecated": True,
        "deprecated_reason": "已废弃，使用'生成Printify订单'替代",
        "page_path": "/scm-orders/[id]",
        "order": 2,
    },
    {
        "step_key": "create_fulfillment_orders",
        "button_key": "create_printify_order",
        "button_label": "创建Printify订单",
        "button_action": "/api/v1/printify/orders",
        "http_method": "POST",
        "is_recommended": False,
        "is_deprecated": True,
        "deprecated_reason": "已废弃，从核心订单直接创建的方式不再使用",
        "page_path": "/orders/[id]",
        "order": 3,
    },

...
const testSaveOrder = async () => {
  try {
    console.log('🧪 开始测试保存 Shopify 订单到数据库...');
    ...
    console.log('📤 发送请求到前端 API...');
    
    // 调用前端 API
    const response = await fetch('http://localhost:3000/api/shopify-orders/', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer test-token' // 使用测试 token
      },

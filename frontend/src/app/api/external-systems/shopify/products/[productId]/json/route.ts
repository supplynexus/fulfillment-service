...
const logger = createLogger('api.shopify-product-json');
...
export async function GET(
  ...
  try {
    ...
    const externalSystemHashid = searchParams.get('external_system_hashid');

    if (!externalSystemHashid) {
      return NextResponse.json(
        { error: 'Missing external_system_hashid parameter' },
        { status: 400 }
      );
    }
    ...
    const bodyString = '';
    const backendPath = `/api/v1/external-systems/shopify/${externalSystemHashid}/products/${productId}/json`;

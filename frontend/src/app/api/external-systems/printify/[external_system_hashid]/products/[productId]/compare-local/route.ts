...
const logger = createLogger('api.printify-product-compare-local');
...
export async function GET(
  ...
  try {
    ...
    const bodyString = '';
    const backendPath = `/api/v1/external-systems/printify/${external_system_hashid}/products/${productId}/compare-local`;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.detail || 'Failed to compare with local DB' },
        { status: response.status }
      );
    }
    ...
    logger.error('Error comparing Printify product with local DB', {

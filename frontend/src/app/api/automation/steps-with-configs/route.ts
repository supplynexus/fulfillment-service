...
const logger = createLogger('api.automation.steps-with-configs');

export async function GET(request: NextRequest) {
  const startTime = Date.now();

  try {
    frontendLogger.info('🚀 开始获取自动化步骤及配置');
    ...
    const backendPath = category
      ? `/api/v1/automation/steps-with-configs?category=${category}`
      : '/api/v1/automation/steps-with-configs';
    const bodyString = '';
    ...
    frontendLogger.info('✅ 获取自动化步骤及配置成功', {
      count: data.length,
      duration,
    });
    ...
    logger.requestComplete(
      'GET',
      '/api/automation/steps-with-configs',
    ...
    const duration = Date.now() - startTime;
    frontendLogger.error('❌ 获取自动化步骤及配置失败', {
      error: error.message,
      duration,
    });

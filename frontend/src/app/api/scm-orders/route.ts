...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.info('SCM order creation request', { requestBody });

    // 对于 POST 请求，签名字符串中的 body 是 JSON 字符串
    const bodyString = JSON.stringify(requestBody);
    const timestamp = Math.floor(Date.now() / 1000);
    const nonce = Math.random().toString(36).substring(2, 15);

    // 构建签名字符串 - 后端实际接收到的路径是 /api/v1/scm-orders/
    const signatureString = `POST/api/v1/scm-orders/${timestamp}${nonce}${tenantName}${bodyString}`;
    ...
    // 构建后端请求 URL
    const backendUrl = `${process.env.BACKEND_API_URL}/api/v1/scm-orders`;

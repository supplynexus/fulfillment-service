...
export async function POST(
  ...
  try {
    ...
    frontendLogger.info('🔄 尝试生成 Printify 订单', { scmOrderHashid, selectedItems: requestBody.selected_items?.length });
    ...
    frontendLogger.info('➡️ 转发生成 Printify 订单请求到后端', { backendUrl, scmOrderHashid, selectedItemsCount: requestBody.selected_items?.length });
    ...
    frontendLogger.info('✅ Printify 订单生成成功', { scmOrderHashid, printifyOrderId: data.printify_order_id, selectedItemsCount: data.selected_items_count });
    return NextResponse.json(data);

  } catch (error: any) {
    frontendLogger.error('❌ 生成 Printify 订单 API 错误', { error: error.message });
    return NextResponse.json(

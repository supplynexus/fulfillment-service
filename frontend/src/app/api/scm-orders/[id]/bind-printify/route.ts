...
export async function POST(
  ...
  try {
    frontendLogger.info('🚀 开始处理 SCM 订单绑定 Printify 订单请求', { scmOrderId });
    ...
    const signatureString = `POST/api/v1/scm-orders/${scmOrderId}/bind-printify-order${timestamp}${nonce}${tenantName}${bodyString}`;
    ...
    // 构建后端URL
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/scm-orders/${scmOrderId}/bind-printify-order`;
    ...
    frontendLogger.info('✅ SCM 订单绑定 Printify 订单成功', {
      scmOrderId,
      status: backendResponse.status,
      duration,
    });
    ...
    const duration = Date.now() - startTime;
    frontendLogger.error('❌ SCM 订单绑定 Printify 订单失败', {
      scmOrderId,
      error: error.message,
      duration,
    });

...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 构建签名字符串
    const signatureString = `GET/api/v1/products/mappings/${timestamp}${nonce}${tenantName}${bodyString}`;
    ...
    // 构建后端请求 URL
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/v1/products/mappings/?${backendParams.toString()}`;

...
function PrintifyOrdersPage() {
  ...
  const fetchOrders = useCallback(
    async (store: PrintifyStore, page: number = 1) => {
      if (!store) return;
    ...
    try {
      ...
      if (response.data.success && response.data.order) {
        ...
        frontendLogger.info('✅ Printify 订单详情获取成功', {
          ...
          appOrderId: order.app_order_id,
          hasShippingInfo: !!(
            orderDetails.tracking_number || orderDetails.tracking_url
          ),
        });
      } else {
                            ...
                            <TableCell>
                              <Box>
                                <Typography variant='body2' fontWeight='medium'>
                                  {order.address_to?.first_name || 'N/A'}{' '}
                                  {order.address_to?.last_name || ''}
                                </Typography>
                                <Typography
                                  variant='caption'
                                ...
                                <Typography
                                  ...
                                  color='text.secondary'
                                >
                                  {order.address_to?.city || 'N/A'},{' '}
                                  {order.address_to?.country || 'N/A'}
                                </Typography>
                              </Box>
                            </TableCell>
                            <TableCell>
                ...
                <Box>
                      ...
                      {selectedOrder.line_items.map((item, index) => (
                        ...
                        >
                          <Box flex={1}>
                            ...
                            <Typography variant='body2' color='text.secondary'>
                              数量: {item.quantity} | SKU:{' '}
                              {item.metadata?.sku || 'N/A'}
                            </Typography>
                            <Typography
                              variant='caption'
                              color='text.secondary'
                  ...
                  <Card sx={{ mb: 2 }}>
                    <CardContent>
                      ...
                      <Typography variant='body1'>
                        {selectedOrder.address_to?.first_name || 'N/A'}{' '}
                        {selectedOrder.address_to?.last_name || ''}
                      </Typography>
                      <Typography variant='body2' color='text.secondary'>
                        {selectedOrder.address_to?.email || 'N/A'}
                        {selectedOrder.address_to?.phone &&
                          ` | ${selectedOrder.address_to.phone}`}
                      </Typography>
                      <Typography variant='body2'>
                        {selectedOrder.address_to?.address1 || 'N/A'}
                        {selectedOrder.address_to?.address2 &&
                          `, ${selectedOrder.address_to.address2}`}
                      </Typography>
                      <Typography variant='body2'>
                        {selectedOrder.address_to?.city || 'N/A'},{' '}
                        {selectedOrder.address_to?.region || 'N/A'}{' '}
                        {selectedOrder.address_to?.zip || 'N/A'}
                      </Typography>
                      <Typography variant='body2'>

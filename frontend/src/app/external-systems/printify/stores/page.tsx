...
function PrintifyStoresPage() {
  ...
  const handleTestConnection = async (store: PrintifyStore) => {
    try {
      ...
      const response = await frontendApi.post(
        '/api/external-systems/printify/test-connection',
        {
          access_token: store.credentials.access_token,
          shop_id: shopId,
  ...
  const handleViewProducts = async (store: PrintifyStore) => {
    ...
    try {
      ...
      const response = await frontendApi.get(
        `/api/external-systems/printify/${externalSystemHashid}/products`
      );
      ...
      setError(
        '获取商品列表失败: ' + (err.response?.data?.detail || err.message)
      );
    } finally {
  ...
  const handleViewOrders = async (store: PrintifyStore) => {
    ...
    try {
      ...
      setError(
        '获取订单列表失败: ' + (err.response?.data?.detail || err.message)
      );
    } finally {
                ...
                <Box display='flex' gap={1} flexWrap='wrap'>
                  <IconButton
                    ...
                    title={
                      ...
                      store.credentials?.shop_id
                        ? '测试连接'
                        : '该店铺未配置 access_token 或 shop_id，无法测试连接'
                    }
                  >
                    <TestConnectionIcon />
                  </IconButton>
                  ...
                  <IconButton
                    ...
                    disabled={
                      !store.credentials?.shop_id ||
                      (store.credentials?.shop_id
                        ? syncingOrders[store.credentials.shop_id]
                        : false)
                    }
                    title={
                      store.credentials?.shop_id
                        ? '同步订单到数据库'
                        : '该店铺未配置 shop_id，无法同步订单'
                    }
                  >
                    {store.credentials?.shop_id &&
                    syncingOrders[store.credentials.shop_id] ? (
          ...
          <Box sx={{ pt: 2 }}>
            ...
            {testingConnection && (
              ...
              >
                ...
                <Typography variant='body2'>正在测试连接...</Typography>
              </Box>
            )}
            ...
            <Box sx={{ mt: 2, p: 2, bgcolor: 'grey.100', borderRadius: 1 }}>
              <Typography variant='body2' color='text.secondary'>
                🔍 Debug: testResult={testResult ? '有值' : 'null'},
                testingConnection={testingConnection.toString()}, forceRender=
                {forceRender}, showDialog={showTestResultDialog.toString()}
              </Typography>
            </Box>
          </Box>
        </DialogContent>
                ...
                <Card key={order.id} sx={{ mb: 2 }}>
                  <CardContent>
                    <Typography variant='h6'>
                      订单 #{order.order_number || order.id || 'N/A'}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                      状态: {order.status || '未知'}
                    </Typography>
                    ...
                    <Typography variant='body2' color='text.secondary'>
                      ...
                      {order.shipping_address
                        ? `${order.shipping_address.first_name || ''} ${order.shipping_address.last_name || ''}, ${order.shipping_address.address1 || ''}, ${order.shipping_address.city || ''}, ${order.shipping_address.country || ''}`
                            .trim()
                            .replace(/^,\s*|,\s*$/g, '')
                        : '无收货地址信息'}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                      创建时间:{' '}
                      {order.created_at ? formatDate(order.created_at) : '未知'}
                    </Typography>
                  </CardContent>
                </Card>
              ))}
